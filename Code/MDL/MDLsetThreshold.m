function [Ldist,Lvals]=MDLsetThreshold(V,precision,Wstore,Hstore,rValues,tW,tH,options)
nRuns=length(Wstore);
Ldist=zeros(nRuns,6);% Lall is Ltot,Le,Lw,Lh %%%% STORE THE LENGTHS FOR ALL THE COMPONENTS
Lvals=zeros(nRuns,6);
V=V/max(max(V));%%%% 
[m,n]=size(V);
plotNum=1;
...
for i=1:nRuns
    ...
    %%% NOW THE W+ TERMS
    [Wvals,Wlocs]=histcounts(W2,'binwidth',precision); % Wlocs CONTAINS EDGES OF THE BINS
    WvalsProb=Wvals/(sum(Wvals)); % WvalsProb ARE THE PROBABILITIES DRAWN DIRECTLY FROM THE HISTOGRAMS
    ...
    Wlocs2=[(Wlocs(1)-precision),Wlocs(1:end)+precision]; % Wlocs2 contains centres of histograms
    Wbinned=interp1(Wlocs2,Wlocs2,W3,'nearest'); % Each term in the W matrix is set to its nearest histgram centre, e.g. it is rounded to the relevant precision
    Wbinned(W0sLoc)=0;Wbinned=reshape(Wbinned,m,r); % Zero values terms are set and Wbinned returned to m-by-r matrix
    ...
    WdistCumu=gamcdf(WlocsCumu(2:end),parW(1),parW(2)); % The cumulative probability density is found, for comparison with WvalsCumu2
    ...
    %%%%%%%%%%%%%%%%%%%%%%%% START CALLING FOR PLOTS %%%%%%%%%%%%%%%%%%%%%%
    
    if(options.eGaussianCumulative)
        eGaussianCumulativePlot(ElocsCumu,pEcumu,EvalsCumu2,plotNum)
        plotNum=plotNum+1;
    end
    if(options.wGammaCumulative)
        wGammaCumulativePlot(WlocsCumu,WdistCumu,WvalsCumu2,plotNum)
        plotNum=plotNum+1;
    end
    if(options.hGammaCumulative)
        hGammaCumulativePlot(HlocsCumu,HdistCumu,HvalsCumu2,plotNum)
        plotNum=plotNum+1;
    end
    if(options.eGaussian)
        eGaussianPlot(Elocs,EdistProb,EvalsProb,plotNum)
        plotNum=plotNum+1;
    end
    if(options.wGamma)
        wGammaPlot(Wlocs,WdistProb,WvalsProb,plotNum)
        plotNum=plotNum+1;
    end
    if(options.hGamma)
        hGammaPlot(Hlocs,HdistProb,HvalsProb,plotNum)
        plotNum=plotNum+1;
    end
...
if(options.lValuesNorm)
    lValuesNormPlot(rValues,Ldist,Lvals,m,plotNum)
end
...
function lValuesNormPlot(rValues,Ldist,Lvals,m,plotNum)
Wdist=Ldist(:,3)+Ldist(:,4);Hdist=Ldist(:,5)+Ldist(:,6);
Wvals=Lvals(:,3)+Lvals(:,4);Hvals=Lvals(:,5)+Lvals(:,6);
x=rValues/m;
y1=Ldist(:,1)/max(Ldist(:,1));y2=Ldist(:,2)/max(Ldist(:,2));
y3=Wdist/max(Wdist);y4=Hdist/max(Hdist);
y11=Lvals(:,1)/max(Lvals(:,1));y22=Lvals(:,2)/max(Lvals(:,2));
y33=Wvals/max(Wvals);y44=Hvals/max(Hvals);
figure(plotNum)
plot(x,y1,'k-')
hold on
plot(x,y2,'k--')
...
Ltot2=Lvals(:,1);Le2=Lvals(:,2);Lw2=Lvals(:,3);Lh2=Lvals(:,4);
